...
public class MyFastPartitioner implements IDocumentPartitioner,
	...
	/**
	 * The cached positions from our document, so we don't create a new array
	 * every time someone requests partition information.
	 */
		...
		try {
			...
			while (!token.isEOF()) {
				...
				if (isSupportedContentType(contentType)) {
					TypedPosition p = new TypedPosition(
		...
		try {
			...
			if (first > 0) {
				...
				if (partition.includes(reparseStart)) {
					...
					if (e.getOffset() == partition.getOffset()
							+ partition.getLength())
...
//					0);
			...
			for (int i = startIndex; i < endIndex; i++) {
				...
				if ((includeZeroLengthPartitions && overlapsOrTouches(gap,
					...
					start = Math.max(offset, gapOffset);
							...
							.min(endOffset, gap.getOffset() + gap.getLength());
	...
	/*
	 * @see
	 * org.eclipse.jface.text.IDocumentPartitionerExtension3#startRewriteSession
